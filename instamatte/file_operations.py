...
def expand_brace_pattern_into_individual_patterns(pattern: str) -> List[str]:
    ...
    return [
        f"{prefix}{extension}{suffix}" 
        for extension in individual_extensions
    ]
...
def discover_images_matching_pattern(work_directory: Path, pattern_specification: str) -> Set[Path]:
...
def generate_unique_output_filename(output_directory: Path, original_image_path: Path) -> Path:
    ...
    while True:
        incremented_filename = f"{original_stem}_{collision_counter}{original_suffix}"
...
def ensure_directory_exists_and_accessible(directory_path: Path) -> None:
    ...
    try:
        ...
        console.print(f"[red]Error creating directory {directory_path}: {filesystem_error}")
        raise
...
def validate_work_directory_exists(work_directory_path: Path) -> None:
    ...
    if not work_directory_path.exists():
        ...
        raise FileNotFoundError(f"Work directory does not exist: {work_directory_path}")
    
    if not work_directory_path.is_dir():
        console.print(f"[red]Path is not a directory: {work_directory_path}")
        raise NotADirectoryError(f"Path is not a directory: {work_directory_path}")
